...
HeaderService.prototype.getEndHash = function(tip, blockCount, callback) {
  ...
  stream.on('end', function() {
    ...
    // Fix by RanchiMall to stop the node from stopping 
    if (results.length != numResultsNeeded) {
      log.error('getEndHash returned incorrect number of results. Result Needed is ' + numResultsNeeded + ' and results.length is ' + results.length) ;
    }
